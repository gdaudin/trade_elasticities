...
A larger share of world trade is conducted at short distances now that in the 1960s, despite technological developments in transportation and communication, e.g. the airplane, the container, and the internet that should have led to the ``death of distance'' by the end of the 20\textsuperscript{th} century.
\footnote{\cite{Cairncross1997,Levinson2006,Friedman2007}}
This increase in the distance elasticity of trade has been dubbed the  ``distance puzzle''.
...
We make the simple point that the flattening out of the world may go hand in hand with a persistent impeding effect of distance on trade if consumers perceive product bundles shipped out by each country to the world market as increasingly substitutable.
\footnote{Equation (4) in \cite{Head2013}: the distance effect is a product of two elasticities, the elasticity of trade to trade costs and the elasticity of trade costs to distance.
We investigate the evolution of the former.}
Figures \ref{fig:vioplot} and \ref{fig:fall_of_sd} are the simplest way to make our point. More and more countries are exporting more and more products, so that the export bundle of all countries is converging toward the entire range of goods.
This is robust to the examination of a more limited sample of exporters.
We work with the canonical \cite{Anderson2003} framework to quantify the contribution of this mechanism.
We find that the non-decreasing distance elasticity of trade can be explained \if 0 is fully attributable to \fi by the increasing similarity of exporter-specific product bundles.
...
\begin{figure}[h!]
	...
	\begin{center}
		...
		\fbox{\includegraphics[trim = 0mm 0mm 0mm 9.5mm, clip, height=2.5in]{vioplot.pdf}}
	\end{center}
\end{figure}
...
\begin{figure}[h!]
	...
	\begin{center}
		\setlength{\fboxrule}{1pt} %makes border lines thick
		\setlength{\fboxsep}{.1in} %increases distance to border
		\fbox{\includegraphics[trim = 0mm 0mm 0mm 9.5mm, clip, height=2.5in]{Fall_of_SD.pdf}}
	\end{center}
\end{figure}
...
Contrary to the canonical loglinear approach, this estimator provides consistent estimates and is robust to rounding error and overdispersion which are both likely features of trade data.
\footnote{\cite{SantosSilva2011} and \cite{Fally2015} provide evidence on desirable properties of the PPML.
...
They conjectured that the distance puzzle was an artefact of reduction in this bias through the extensive margin of trade.
\footnote{This discussion leaves open the question of the estimator which correctly captures the level of the distance elasticity.
...
IL Y A UN SOUCI AVEC CETTE CITATION. JE VOUS LAISSE REGARDER ?
