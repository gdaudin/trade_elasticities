...
A first difficulty arises when the trade flow is observed but information on quantities is missing, and it is therefore not possible to compute the unit value. Figure \ref{fig:missing_uv} indicates how often that happens from 1962 to 2013, by level of aggregation. The problem arises for a small share of the total trade value (never more than 25\%).
