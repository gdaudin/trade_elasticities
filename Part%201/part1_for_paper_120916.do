*This program adjusted on Sept 12 to stable sample defined in 1962-2013
*and to suppress FTA controls*
...
foreach s of local sample {
	...
	replace `s'=-(exp(`s'))
*growth rate: 
*0.09% per year for full sample (sign. at 10%): 4.5% increase in coef (marg significant)
*0.3% per year for superbal sample (sign at 1%): 16.6% increase in coef 
...
*0.292% per year for superbal sample (sign at 1%): 16% increase in coef 
	quietly graph twoway (scatter estim_`s' ci_`s'_high ci_`s'_low year, msymbol(smcircle smcircle_hollow smcircle_hollow) mcolor(gs4 gs8 gs8)) /*
	*/ (line `s' year, lcolor(red) lpattern(dash) ylabel(-.8(.1)-.4) ytick(-.8(.2)-.4)), legend(order(1 2 4) label(1 "coef_estim") /*
	*/ label(2 "conf_int") label(4 "geometric fit")) xtitle(year) ytitle(distance elasticity) title("`s' sample") saving(`s')
}
...
foreach s of local sample {
	...
	replace `s'=-(exp(`s'))
*growth rate: 
*0.4% per year for full sample (sign. at 1%): 22% increase in coef [explains 89% of tot variation]
*0.51% per year for superbal sample (sign at 1%): 34% increase in coef 
