...
foreach agg of num 5(-1)1 {
	...
	foreach pays_dest in `r(list)' {
		...
		*/ commerce_paire_avec_uv_agg`agg'=total(value_`year'_avec_uv_agg`agg')
		...
		label var pour_compter_`agg' 	 "Number of potential trade flows (only in pre-existing sectors and partners for a destination)" 
...
*this includes lacking uv for existing trade but also lacking trade and uv
		...
		label var pour_compter_sscommerce_`agg' "ztf Number of trade flows without value (only in pre-existing sectors and partners for a destination)"	
